...
def make_encoder_layers(
    input_size: int, latent_dim: int = 100, hidden_channel: int = 128, last_act: str = None
) -> list:
    # mnist, fmnist
    if input_size == 28:
        layers = [
            ...
            ConvLayer(hidden_channel, hidden_channel * 2, 4, 2, 1, True, "leakyrelu"),
            ConvLayer(hidden_channel * 2, hidden_channel * 4, 4, 2, 1, True, "leakyrelu"),
            ConvLayer(hidden_channel * 4, latent_dim, 3, 1, 0, False, last_act),
        ]
    # cifar10
    elif input_size == 32:
        layers = [
            ...
            ConvLayer(hidden_channel, hidden_channel * 2, 4, 2, 1, True, "leakyrelu"),
            ConvLayer(hidden_channel * 2, hidden_channel * 4, 4, 2, 1, True, "leakyrelu"),
            ConvLayer(hidden_channel * 4, latent_dim, 4, 1, 0, False, last_act),
        ]
    else:
        raise ValueError
    return layers
